...
- 2026-05-11 20:50 — **Folder renamed: `marquee-studio/` → `green-country-web/`.** Local rename done, internal refs swept (package.json, package-lock, scripts/README, score-site.mjs, cron prompt). Vercel project itself still NAMED `marquee-studio` so live URLs (`marquee-studio-XXX.vercel.app`) are unchanged. `vercel link --project marquee-studio` ran in new folder; .vercel/project.json points to the same project. Both `green-country-web/` AND `foam-dial-pro/` now imported into workspace repo for the first time.
